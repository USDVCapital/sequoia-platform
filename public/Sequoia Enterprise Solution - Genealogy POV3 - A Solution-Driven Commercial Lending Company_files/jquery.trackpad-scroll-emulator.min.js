...
/*Custom JS*/
function customDragScrollInit() {
	/*Sample Layout * tse-gridview for grideView c# code
	*<div class="tse-scrollable tse-scrollable-cls horizontal tse-gridview">
	* <div class="tse-content">
	* </div>
	*</div>
	*/
	var pThis = this;
	var addGrabMousePointer = function() {
		jQuery('.drag-handle').on('mousedown', function mouseState(e) {
			if (e.type == "mousedown") {
				jQuery('html').addClass('drag-handle-active');
			}
		});
		jQuery('html').on('mouseup', function mouseState(e) {
			if (e.type != "mousedown") {
				//code triggers on hold
				jQuery('html').removeClass('drag-handle-active');
			}
		});
	}
	var dragScrollInit = function() {
		jQuery('.tse-gridview').each(function () {
			var childDiv = jQuery(this).children('div');
			if (!childDiv.attr('class')) {
				childDiv.addClass('tse-content');
			}
			jQuery(this).TrackpadScrollEmulator({ autoHide: false });
			addGrabMousePointer();
		});
	}
	pThis.init = function() {
...
var cdsInit = new customDragScrollInit();
