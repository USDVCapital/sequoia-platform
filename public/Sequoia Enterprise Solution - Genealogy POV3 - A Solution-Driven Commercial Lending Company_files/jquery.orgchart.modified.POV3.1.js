 ...
 * Modified by Carl Calaquian to fix vertical chart on demand load and added new parameters for other types of events.
 */
'use strict';
...
(function (factory) {
    ...
    OrgChart.prototype = {
            ...
            var $chart = this.$chart = $('<div>', {
                ...
                'click': function (event) {
                    if (!$(event.target).closest('.node').length) {
                        $('.node.focused').removeClass('focused');
                    }
                }
            });
        ...
        setChartScale: function ($chart, newScale) {
            var chartEl = $chart.$chart;
            if (!chartEl) {
                chartEl = $chart;
            }
            var opts = chartEl.data('options');
            var lastTf = chartEl.css('transform');
            var matrix = '';
            var targetScale = 1;
            if (lastTf === 'none') {
                chartEl.css('transform', 'scale(' + newScale + ',' + newScale + ')');
            } else {
                matrix = lastTf.split(',');
                if (lastTf.indexOf('3d') === -1) {
                    targetScale = Math.abs(window.parseFloat(matrix[3]) * newScale);
                    if (targetScale > opts.zoomoutLimit && targetScale < opts.zoominLimit) {
                        chartEl.css('transform', lastTf + ' scale(' + newScale + ',' + newScale + ')');
                        var yOffsetValue = ((orgChart.$chart[0].getBoundingClientRect().height - orgChart.$chart[0].offsetHeight) * .5).toFixed(2);
                        var xOffsetValue = ((orgChart.$chart[0].getBoundingClientRect().width - orgChart.$chart[0].offsetWidth) * .5).toFixed(2);
                        var newLastTf = chartEl.css('transform').split(', ');
                        newLastTf = matrix;
                        if (orgChart.$chart[0].className.indexOf('vertical-view') !== -1) {
                            newLastTf[4] = xOffsetValue;
                        }
                        newLastTf[5] = yOffsetValue + ')';
                        lastTf = newLastTf.join();
                        chartEl.css('transform', lastTf + ' scale(' + newScale + ',' + newScale + ')');
                    }
                } else {
                    targetScale = Math.abs(window.parseFloat(matrix[1]) * newScale);
                    if (targetScale > opts.zoomoutLimit && targetScale < opts.zoominLimit) {
                        chartEl.css('transform', lastTf + ' scale3d(' + newScale + ',' + newScale + ', 1)');
                    }
                }
            }
        },
        ...
        loopChart: function ($chart) {
            ...
            var nodeId = $tr.length > 0 ? $tr.find('.node')[0].id : 0;
            var subObj = { 'id': nodeId };
        ...
        hideChildrenEnd: function (event) {
            ...
            if (!this.opts.alwaysShowBottomEdge && this.isInAction($node)) {
                this.switchVerticalArrow($node.children('.bottomEdge'));
            }
            if (this.opts.alwaysShowBottomEdge) {
                $node.children('.bottomEdge').removeClass('fa-chevron-up').addClass('fa-chevron-down');
            }
            scrollerScrolTo($node[0]);
        ...
        showChildrenEnd: function (event) {
            ...
            if (this.opts.alwaysShowBottomEdge) {
                for (var i = 0; i < event.data.animatedNodes.length; i++) {
                    var $bottomEdge = $(event.data.animatedNodes[i]).children('.bottomEdge');
                    $bottomEdge.removeClass('fa-chevron-up').addClass('fa-chevron-down');
                }
                $node.children('.bottomEdge').removeClass('fa-chevron-down').addClass('fa-chevron-up');
            }
            if (event.data.animatedNodes.length < 2) {
                scrollerScrolTo($node[0], event.data.animatedNodes[event.data.animatedNodes.length - 1]);
            } else {
                scrollerScrolTo($node[0]);
            }
        },
        ...
        showSiblingsEnd: function (event) {
            ...
            if (this.opts.alwaysShowBottomEdge) {
                for (var i = 0; i < event.data.visibleNodes.length; i++) {
                    var $bottomEdge = $(event.data.visibleNodes[i]).children('.bottomEdge');
                    $bottomEdge.removeClass('fa-chevron-up').addClass('fa-chevron-down');
                }
            }
        },
        ...
        nodeEnterLeaveHandler: function (event) {
            ...
            if (event.type === 'mouseenter') {
                ...
                $node.children('.edge').removeClass('fa-chevron-down fa-chevron-up fa-chevron-right fa-chevron-left');
                if (this.opts.alwaysShowBottomEdge) {
                    if ($bottomEdge.length) {
                        flag = this.getNodeState($node, 'children').visible;
                        $bottomEdge.toggleClass('fa-chevron-down', !flag).toggleClass('fa-chevron-up', flag);
                    ...
                    $node.children('.edge').removeClass('fa-chevron-down');
                }
            ...
            $.ajax({ 'url': url, 'dataType': 'json' })
                .done(function (data) {
                    if (that.$chart.data('inAjax')) {
                        ...
                        if (data[rel]) {
                            var lastChild = data[rel][data[rel].length - 1];
                        }
                        scrollerScrolTo($edge[0], lastChild);
                    }
                })
                .fail(function () {
                ...
                .always(function () {
                    ...
                    if (that.options.ajaxCompleted && typeof that.options.ajaxCompleted === 'function') {
                        that.options.ajaxCompleted($edge);
                    }
                });
        },
        // load new nodes by ajax

        loadVChildNodes: function (url, $parentNode) {
            var that = this;
            var opts = this.options;
            $.ajax({ 'url': url, 'dataType': 'json' })
                .done(function (data) {
                    if (that.$chart.data('inAjax')) {
                        if (data.children != null && data.children.length) {
                            //var $appendTo = $parentNode.parent();
                            // that.addChildren($parentNode.parent(), data[rel]);
                            //that.buildHierarchy($appendTo, data, true);
                            // this.buildHierarchy()
                            var $nodesLayer = $('<ul>');
                            //$parentNode.find('.toggleBtn').first().addClass('fa-minus-square').removeClass('fa-plus-square');
                            $parentNode.append($nodesLayer);
                            if (data.children) {
                            ...
                            $.each(data.children, function () {
                                ...
                                //this.level = level + 1;
                                that.buildHierarchy($nodeCell, this, true);
                            });
                ...
                .always(function () {
                    $parentNode.children().first().find('.spinner').remove();
                    $parentNode.children().first().find('.toggleBtn').removeClass('hidden');
                    $parentNode.children().first().children().removeAttr('style');
                    //$parentNode.children().find('.toggleBtn').addClass('fa-plus-square').removeClass('fa-minus-square');
                    //endLoading($parentNode);
                    that.$chart.data('inAjax', false);
                    $('.oc-export-btn' + (that.options.chartClass !== '' ? '.' + that.options.chartClass : '')).prop('disabled', false);




                    if (that.options.ajaxCompleted && typeof that.options.ajaxCompleted === 'function') {
                        that.options.ajaxCompleted($parentNode);
                    }
        ...
        topEdgeClickHandler: function (event) {
            ...
            var $parent1 = $node.closest('table').closest('tr').siblings(':first').find('.node');
            manageGotoTopArrow($parent1.attr('id'));
        },
        //
        bottomEdgeClickHandler: function (event) {
            ...
            if (this.getHierarchy().id === event.target.parentNode.id && orgChart.$chart[0].className !== 'vertical-view') {
                orgChart.$chart[0].parentNode.classList.add('recalculating');
            }
        ...
        hEdgeClickHandler: function (event) {
            ...
            scrollerScrolTo($node[0], false, goToLeftMost);
            //manageGotoTopArrow($node[0].id);
        },
        //
        ...
        toggleVNodes: function (event) {
            ...
            if (($descendants.length || 0) == 0) {
                event.stopPropagation();
                var $vEdge = $(event.target);
                var $node = $(event.delegateTarget);
                var opts = this.options;

                if (this.startLoading($vEdge)) {
                    var opts = this.options;
                    var url = $.isFunction(opts.ajaxURL.children) ? opts.ajaxURL.children($node.data('nodeData')) : opts.ajaxURL.children + $node[0].id;
                    this.loadVChildNodes(url, $vEdge.parent().parent());
                }
        ...
        // create node
        createNode: function (data, forceVertical) {
            var that = this;
            var opts = this.options;
            ...
            var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (data[opts.nodeId] ? ' id="' + data[opts.nodeId] + '"' : '') + (data.parentId ? ' data-parent="' + data.parentId + '"' : '') + 'relationship="' + data.relationship + '"' + '>')
                .addClass((data.className || '') + ' node ' + (level > opts.visibleLevel ? ' slide-up' : ''));
            ...
            if (opts.verticalLevel && level >= opts.verticalLevel || forceVertical) {
                $nodeDiv.addClass('vnode');
                if (((level + 1) > opts.verticalLevel || forceVertical) && Number(flags.substr(2, 1))) {
                    var icon = (level + 1 > opts.visibleLevel || (data.children == null && typeof opts.ajaxURL !== 'undefined')) ? 'plus' : 'minus';
                ...
                if (Number(flags.substr(2, 1))) {
                    ...
                    if (opts.alwaysShowBottomEdge) {
                        var $bottomEdge = $nodeDiv.children('.bottomEdge');
                        var flag = this.getNodeState($nodeDiv, 'children').visible;
                        $bottomEdge.toggleClass('fa-chevron-down', flag)
                        $bottomEdge.toggleClass('fa-chevron-up', !flag);
                    }
                }
            }
        ...
        buildHierarchy: function ($appendTo, data, forceVertical) {
            forceVertical = forceVertical || false;
            var that = this;
            var opts = this.options;
            var level = 0;
            if (data.level) {
                ...
                level = data.level = $appendTo.parentsUntil('.orgchart', 'li').length + opts.verticalLevel;
            }
            // Construct the node
            ...
            if (Object.keys(data).length > 2) {
                $nodeDiv = this.createNode(data, forceVertical);
                if (opts.verticalLevel && level >= opts.verticalLevel || forceVertical) {
            ...
            if (hasChildren) {
                ...
                var $nodesLayer;
                if (isVerticalLayer || forceVertical) {
                    $nodesLayer = $('<ul>');
                    if (isHidden && (level + 1 > opts.verticalLevel) /* && forceVertical */) {
                ...
                $.each(childrenData, function () {
                    var $nodeCell = (isVerticalLayer || forceVertical) ? $('<li>') : $('<td colspan="2">');
                    $nodesLayer.append($nodeCell);
                    this.level = level + 1;
                    that.buildHierarchy($nodeCell, this, forceVertical);
                });
            }
            if ($nodeDiv != null && opts.alwaysShowBottomEdge) {
                var $bottomEdge = $nodeDiv.children('.bottomEdge');
                if ($bottomEdge.length) {
                    var flag = this.getNodeState($nodeDiv, 'children').visible;
                    $bottomEdge.toggleClass('fa-chevron-down', !flag);
        ...
        addChildren: function ($node, data) {
            ...
            if ($node != null && this.opts.alwaysShowBottomEdge) {
                var $bottomEdge = $node.children('.bottomEdge');
                if ($bottomEdge.length) {
                    var flag = this.getNodeState($node, 'children').visible;
                    if (flag) {
                        $bottomEdge.toggleClass('fa-chevron-up', flag).toggleClass('fa-chevron-down', !flag);
                    }
                }
            }
        },
